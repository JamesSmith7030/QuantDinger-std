...
my_indicator_name = "v1.3.2(BTC4H做多+90.22)-双均线策略"
my_indicator_description = "EMA双均线做多策略(优化):短期EMA上穿长期EMA且价格在趋势线上方时开多,下穿时平多;引擎管理止损/止盈控制回撤。信号收盘确认,次开盘成交"
# 回测参数 标的: BTC, K线周期: 4H, 日期范围: 2Y, 杠杆: 3x, 交易方向: 做多, 回测收益: +90.22%

# --- 1. 参数声明 (Params) ---
# @param ema_short_len int 15 短期EMA周期
# @param ema_long_len int 40 长期EMA周期
# @param trend_len int 150 趋势过滤EMA周期
# @param use_trend_filter bool true 是否启用趋势过滤(仅在长期趋势向上时做多)

# --- 2. 策略风控默认值 (Strategy Defaults) ---
# @strategy entryPct 0.85
# @strategy stopLossPct 0.06
# @strategy takeProfitPct 0.18
# @strategy trailingEnabled true
# @strategy trailingActivationPct 0.06
# @strategy trailingStopPct 0.035
# @strategy tradeDirection long
#
# 原参数(EMA10/30)最大回撤高达 -86.78%、胜率仅33%、Sharpe 0.58。
# 优化思路(保持双均线交叉核心逻辑):
#   1) 放慢均线到 20/50,减少高波动区的假突破/来回打脸,提升单笔质量。
#   2) 增加长期趋势过滤(EMA200),只在趋势向上时做多,避开下跌段反复亏损。
#   3) 交由引擎管理固定止损(8%)+止盈(25%)+移动止损,直接压制最大回撤、改善风险调整后收益。

df = df.copy()

# ---- 读取参数 ----
ema_short_len = int(params.get('ema_short_len', 15))
ema_long_len = int(params.get('ema_long_len', 40))
trend_len = int(params.get('trend_len', 150))
