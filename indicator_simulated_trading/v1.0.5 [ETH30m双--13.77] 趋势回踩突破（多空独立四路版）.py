my_indicator_name = "v1.0.5 [ETH30m双向--13.77] 趋势回踩突破（多空独立四路版）"
my_indicator_description = "v1.0.4 的双向扩展：多头沿用趋势回踩/突破入场；空头为独立镜像建模（下跌趋势 + RSI 弱势 + 向下突破或反弹受阻），不再复用多头离场条件作为开空信号。四路信号显式表达，退出由引擎风控负责，指标侧 close_* 仅做结构性离场。"

# signal_form: four_way
# exit_owner: engine
# flip_mode: R2

# @param fast_len int 34 快线 EMA 周期
# @param slow_len int 96 慢线 EMA 周期
# @param rsi_len int 18 RSI 周期
# @param rsi_floor float 55 开多所需的最低 RSI（动量确认）
# @param rsi_ceiling float 45 开空所需的最高 RSI（弱势确认）
# @param rsi_long_exit float 44 多头动量衰竭离场阈值（RSI 跌破）
# @param rsi_short_exit float 56 空头动量恢复离场阈值（RSI 升破）
# @param atr_len int 20 ATR 周期
# @param atr_filter_mult float 1.8 ATR 极端波动过滤倍数
# @param breakout_len int 20 突破回看根数
# @param pullback_buffer float 0.004 回踩/反弹至快线的容差比例
