...
同一套三支柱框架下支持两种模式，**由用户提示词决定**。除 K 线周期集外，波段的**评分权重/止损口径/趋势确认周期**也随模式调（v1.3.1）；枢轴/20日区间/结构化支撑阻力两模式一致（仍日线锚定）：

| 维度 | 短线（默认） | 波段（swing） |
|------|-------------|---------------|
| 触发提示词 | "生成一份最新的深度分析报告"、"…深度**短线**分析报告"（不含"波段"） | "…生成一份最新的深度**波段**分析报告"（含"**波段**"） |
| 定位 | 1-3 天持仓、日频，15m 做入场择时 | 数天-数周持仓 |
| 多周期 RSI/MACD | `15m/1H/4H/1D` | `1H/4H/1D/1W`（删 15m、加周线 1Wutc 定趋势） |
| 三支柱权重（宏观/量价/衍生） | `30/40/30` | `40/35/25`（宏观加权，更看估值） |
| 止损/止盈 ATR | 日线 ATR(14)，2×/3× | **周线 ATR(14)**（额外拉 ATR14W@1Wutc），2×/3× |
| 趋势共振周期（decide） | `4H + 1D` MACD 同向 | `1D + 1W` MACD 同向 |
| 报告标题 / 文件名 | 「深度分析报告」/ `<sym>_report_<ts>.md` | 「深度**波段**分析报告」/ `<sym>_swing_report_<ts>.md` |

判定规则：**提示词出现"波段"→ swing 模式；否则默认 short（短线）**。`build_report.py --mode auto` 从数据文件 `=== MODE ===` 标记自动识别。
...
下面第 1A 步的命令清单是 `pull_okx_data.py`（短线口径）内置取数的对照参考；波段模式把 RSI/MACD 的 `15m` 换成 `1Wutc`、**并额外拉一条 ATR14@`1Wutc`（周线止损用）**（写周线一律 `1Wutc`，`1W` 会 HTTP 400），其余日线指标不变。
