...
updated: 2026-07-15
description: >-
  加密货币深度分析与专业报告生成技能。获取 OKX 实时行情、计算多周期技术指标、
  按三支柱框架评分（短线30%+40%+30%，波段40%+35%+25%），生成结构化 Markdown 专业报告。
  支持单标的与多标的；
...
<!-- 变更日志（维护用）：
  v1.4.0 (2026-07-15) — 长期实战 review 后的语义与可靠性升级：①把原综合分明确为**市场温度评分/分区**，只描述估值与热度；综合信号改为可执行方向（确认做多=BUY、确认做空=SELL、其余=NEUTRAL/HOLD），消除“BUY+做空”；②多周期共识改为多空对称，4 个空头周期正确显示 SELL/100%；③做空路径的理由、止损、RR、HOLD 复核措辞与终端汇总统一走镜像值；④币种输入白名单、混合模式拒绝、批量部分失败返回非零；⑤固化扩展聊天汇总格式并新增标准库回归测试；⑥股票/股票代币自动报告明确列为后续独立迭代，不在当前自动化能力内。
...
当前自动化链路只覆盖**加密货币**，走 OKX 公开行情（**免凭证、免代理**，国内可直连
okx CLI）。分析框架复用本机全局技能 `kline-indicator`（技术分析大师）的三支柱评分法。

> TODO：股票/股票代币自动报告（Yahoo Finance + Bitget + 交易时段 + 独立评分）已列为
> 后续独立迭代。当前保留的股票章节仅是设计资料，**不得宣称端到端自动化可用**。
...
本技能**只做分析/取数、不下单**。当前自动化只执行加密行；股票两行是后续迭代的数据源约束：

| 资产类型 | 唯一取数源 | 禁止替代源 |
|----------|------------|------------|
| 加密货币 | OKX 公开行情；可选 datahub 只补加密宏观/情绪/新闻 | 不用股票数据源替代 |
| 股票（真股，后续 TODO） | **Yahoo Finance 真股 chart 数据** | 当前未实现；禁止 OKX 股票映射永续、Binance RWA、datahub 替代 |
| 股票代币（后续 TODO） | **Bitget 股票代币现货**（首选 R 系） | 当前未实现；禁止 OKX 股票映射永续、Binance RWA、Yahoo 冒充代币价 |
...
> 这样：基础档和增强档共享同一温度/方向契约；增强档只把宏观/情绪/新闻从「--」
> 升级为真实数据，不改变 OKX 技术指标口径。股票与股票代币不走该增强链路。
...
| **股票（真股）** | TSLA / NVDA / AAPL / MSFT、"特斯拉股票"、"英伟达股票" | 当前自动化未实现；说明后续 TODO，不生成伪报告 |
| **股票代币** | 明确说"股票代币"、`RTSLAUSDT`/`RNVDAUSDT`、`xStock` | 当前自动化未实现；说明后续 TODO，不跨源替代 |

加密统一成 OKX instId：
- 现货 ticker / 指标：`BTC-USDT`、`ETH-USDT`、`SOL-USDT`
- 永续（资金费率/持仓量）：`BTC-USDT-SWAP`
- 多标的：逐个循环，**每个加密标的走完取数→评分→落盘并各自独立成报告**。

安全边界：调用 shell 前先把提示词中的标的规范化为 `^[A-Z0-9]{1,20}$`，只把规范化后的
符号传给脚本，禁止把原始用户文本直接拼进命令。两个 CLI 仍会再次校验，形成双层防护。
...
# ① 拉数据到 agent scratchpad/系统临时目录，不要写入 analysis_reports/
python $SKILL/pull_okx_data.py --symbols BTC,ETH,SOL,BNB --out-dir <临时拉取目录> --mode short   # 或 --mode swing
# ② 渲染报告（--mode auto 自动读数据里的 MODE 标记；落盘 analysis_reports/）
python $SKILL/build_report.py  --symbols BTC,ETH,SOL,BNB --in-dir <临时拉取目录> --out-dir analysis_reports
```
...
`市场温度 = 宏观×0.30 + 量价×0.40 + 衍生品×0.30`（0-100）

温度分区：0-20 极度低估/抄底 · 20-40 复苏 · 40-60 中性 · 60-80 过热 · 80-100 狂热。
**温度只回答“便宜/中性/过热”，不直接产生交易方向。** 可执行综合信号由第 2.5 步决定：

- 做多结构 + 做多 RR 闸门通过 → **BUY / 做多**；
- 做空结构 + 做空 RR 闸门通过 → **SELL / 做空**；
- 其余情况 → **NEUTRAL / HOLD**。

因此低估不自动等于 BUY，过热也不自动等于 SELL。

> ⚠️ **诚实第一**：温度与信号必须由真实数据算出，**不得照抄任何样例报告的结论**。
...
- 🔑 **交易方向由分析数据决定，双向（做多/做空），严禁默认做多**：
  - **偏多确认 → 做多表**：现价站上 MA5/MA10，且短线 4H+1D（波段 1D+1W）MACD 柱同正；做多 RR ≥1，超买时 RR 还需 ≥1.2。
  - **偏空确认 → 做空表**（公式镜像，见上「空单」行）：现价跌破 MA5/MA10，且对应趋势周期 MACD 柱同负；做空 RR ≥1，深度超卖时仍 HOLD、不追空。
  - **多空交织、得不到明确方向 → HOLD**：纯中性、或方向因子打架（如破位下行但深度超卖、价跌但衍生品转多）。
- **HOLD 时开仓指南表只显示「当前价格」一行 + 标注 `HOLD`**，不展示建议入场/止损/止盈；表后可列**双向 RR** 和候选复核位，但必须明确写成“非当前建议入场”。
...
1. 首行注明报告数量、快照时间、短线/波段模式、实际数据源档位。
2. 表格至少包含：币、现价、24h、市场温度分、温度分区、可执行信号、
   **做多/做空 RR**、20 日区间位置、可点击报告路径。
3. 若能取得上一轮报告，说明真正发生的市场变化；没有上一轮就明确不做历史比较。
4. 逐币解释为什么 BUY/SELL/HOLD，尤其区分“无方向”和“有方向但位置/RR 不合格”。
5. 给组合层结论、关键位或复核条件，并注明增强档健康状态与缺失数据。
6. 结尾保留“仅供研究，不构成投资建议”。

> RR 展示规则：方向明确时突出该方向 RR；HOLD 时不得把单一 RR 冒充建议，表格写
> `多 <rr_long> / 空 <rr_short>`，正文说明哪一侧只是候选参考。
...
| **市场温度评分** | **<分>/100** |
| **市场温度分区** | **<抄底/复苏/中性/过热/狂热>**（不直接决定开仓方向） |
| **置信度** | <高/中/低 ~xx%> |
| **市场阶段** | <一句话定位> |
| **当前价格** | **$<价>**（24h <涨跌>） |
...
|------|-----|
| 共识方向 | <BUY/MIXED/SELL> |
| 周期一致度 | <xx%>（4 个 RSI 周期的主导方向占比；多空对称） |
| 多周期评分 | <-5 ~ +5> |

---

## 下一步（行动清单）
按当前 <可执行信号> 与市场温度 <分> 的定位，建议的下一步动作（按优先级）：
...
## 股票 / 股票代币分析（后续独立迭代 TODO，当前不执行）

本节仅保留未来实现时的数据源红线和设计资料。当前版本没有 Yahoo/Bitget 自动取数器、
股票评分渲染器和端到端回归测试；收到股票请求时应说明能力待办，**不得照本节手工拼接后
宣称自动报告已实现**。

> 后续迭代范围：Yahoo Finance 真股、Bitget 股票代币、交易时段、溢折价、独立股票三支柱、
> 确定性测试和端到端报告。datahub 不作为股票价格替代源。
...
未来评分可从 `估值×0.30 + 量价×0.40 + 市场×0.30` 起步，但温度分区、执行信号和置信度必须针对股票/股票代币单独验证，**不得直接复用旧版“分数映射 BUY/SELL”逻辑**。诚实第一，禁止照抄样例结论。
...
最终报告不纳入版本库，避免旧行情被误当最新数据。结构回归以
`tests/test_crypto_analysis_report.py` 和临时目录冒烟报告为准；报告必须包含 9 个编号板块、
多周期共识、开仓指南、技术指标 PRO、量化参数明细和免责声明。
...
- 汇总回复：遵守第 4 步的扩展聊天回复契约，明确温度与可执行方向、双向 RR 和数据档位。
