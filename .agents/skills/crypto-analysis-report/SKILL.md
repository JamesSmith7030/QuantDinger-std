...
- 方向跟随综合信号：**BUY 才给做多表，SELL 才给做空表**。
- **若数据无法得到明确交易方向（NEUTRAL / HOLD）**，开仓指南只显示「当前价格」一行，并明确标注 `HOLD`，**不再展示入场/止损/止盈**。
...
| 项目 | 价位 | 说明 |
|------|------|------|
| 交易方向 | <做多 / 做空> | 由综合信号与量价状态决定 |
| 当前价格 | $<价> | <24h 涨跌> |
| 建议入场 | $<入场> | <做多=回踩确认；做空=反弹承压> |
| 止损价 | $<止损> | 按当前方向用 ATR×结构融合计算 |
| 止盈目标 | $<止盈> | 按当前方向用 ATR×结构融合计算 |
| 风险回报比 | 1 : <RR> | 多单 / 空单参考 |

> 若综合结论为 **NEUTRAL / HOLD**，本表**只保留「当前价格」一行**，不展示交易方向以外的执行参数。
