...
1. **综合结论**：一张表给你信号（🟢买入/🟡中性/🔴卖出）、评分（0-100）、置信度、现价。**只看这一块就够做大致判断。**
2. **下一步（行动清单）**：按当前信号给的具体动作——盯哪个价位、该观望还是低吸、止损设哪。
3. **三支柱评分拆解**：为什么是这个分。
   - 宏观周期(30%)：长线贵不贵（AHR999、彩虹图）。**分低=便宜=利多**。
   - 量价因子(40%)：多周期 RSI/MACD/均线说什么。
   - 衍生品(30%)：资金费率（负数=空头多=偏多）、持仓量。
4. **关键价位 / 核心观点 / 风险提示 / 操作倾向**：细节与不同风格的建议。
